...
;                const response = await axios.post(`http://localhost:3000/exams/${id}`);
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        console.log('Submitting...');  // Log before submission
        ...
        try {
           ...
           // console.log('Response:', response);  // Log response
            ...
            alert('Error submitting responses. Please try again later.');
        }
    };
  ...
  //  borderBottom: '1px solid #ddd',
    ...
    <h4 style={{ margin: 0, fontWeight: '500' }}>{currentQuestion.questionText}</h4>
    
    <div style={{ textAlign: 'right' }}>
        <h3 style={{ margin: 0, fontSize: '0.9rem', color: '#4CAF50' }}>
            Mark: {currentQuestion.questionMarks}
        </h3>
        <h3 style={{ margin: 0, fontSize: '0.9rem', color: '#F44336' }}>
            {currentQuestion.negativeMarking 
                ? `Negative Mark: ${(currentQuestion.negativePercentage * currentQuestion.questionMarks)/100}`
                : 'No Negative Mark'}
        </h3>
    </div>
</div>
